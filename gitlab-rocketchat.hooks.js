...
const IGNORE_UNKNOWN_EVENTS = true;
...
See: ${data.object_attributes.url}`,
  ...
  pipelineEvent(data) {
    ...
    if (pipeline.ref !== 'preview' && pipeline.ref !== 'production') {
      return null;
    }
    if (pipeline.status !== 'failed' && pipeline.status !== 'success') {
      return null;
    }
    ...
    return {
      content: {
        ...
        icon_url: USE_ROCKETCHAT_AVATAR ? null : avatar,
        attachments: [
          makeAttachment(user, `pipeline *${build.name}* in stage *${build.stage}* for branch *${pipeline.ref}* returned *${pipeline.status}* for commit [${commit.id.slice(0, 8)}](${commit.url}) made by *${commit.author.name}*`, pipeline_time, STATUSES_COLORS[pipeline.status])
        ]
      }
    };
  }
